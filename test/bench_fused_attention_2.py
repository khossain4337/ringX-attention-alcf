"""
Fused Attention — lean component
=================================
...
This module strips out all benchmarking, plotting, and test scaffolding,
leaving only the reusable attention kernel. It is intended to be used as
a drop-in component within the ringX-attention repo.

Key difference from the original:
  - _attention.forward() now returns (o, M) instead of just o
    so that the LSE (log-sum-exp) is available to the ring attention algorithm.
...
if __name__ == "__main__":
                ...
                for warp_specialize in ([False, True] if enable_ws else [False]):
                    ...
                    bench_configs.append(
                        triton.testing.Benchmark(
                            ...
                            line_arg="provider",
                            line_vals=(
                                (["triton-fp16"] if include_fp16 else []) +
                            ...
                            ylabel="TFLOPS",
                            plot_name=(
                                f"fused-attention-batch{BATCH}-head{N_HEADS}"
                                f"-d{HEAD_DIM}-{mode}-causal={causal}"
    ...
    def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider,
                               device=DEVICE):
        ...
        if "triton" in provider:
            ...
            sm_scale = 1.3
            # attention now returns (o, M) — unpack, use only o for benchmark
            fn = lambda: attention(q, k, v, causal, sm_scale, warp_specialize)[0]
            if mode == "bwd":
                o, _ = attention(q, k, v, causal, sm_scale, warp_specialize)
